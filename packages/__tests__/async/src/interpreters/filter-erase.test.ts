...
import _filterMachine7 from './filter-erase.7.machine';
import _filterMachine8 from './filter-erase.8.machine';
...
describe('Filter and Erase actions', () => {
    ...
    describe('#01 => Filter array of numbers', () => {
      ...
      test('#03 => Add actions', () => {
        service.addOptions(({ assign, filter }) => ({
          actions: {
            addNumbers: assign('numbers', { ADD: ({ payload }) => payload.values }),
  ...
  describe('#02 => Erase action', () => {
    describe('#01 => Erase single context property', () => {
      ...
      test('#03 => Add actions', () => {
        service.addOptions(({ assign, erase }) => ({
          actions: {
            setName: assign('name', { SET_NAME: ({ payload }) => payload.name }),
      ...
      test('#03 => Add actions', () => {
        service.addOptions(({ assign, erase, batch }) => ({
          actions: {
            ...
            clearAll: batch(erase('name'), erase('email'), erase('age')),
          },
        }));
      });
    ...
    describe('#05 => Erase multiple properties (action result)', () => {
      const machine = _machine6;
