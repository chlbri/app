...
describe('AsyncMachine - guardBatch', () => {
  ...
  const machine = createMachine(config, { ...typings }).provideOptions(
    ({ guardBatch, isValue }) => ({
      guards: {
        checkAnd: guardBatch(
          true,
        ...
        checkOr: guardBatch(isValue('context.active', true), {
          or: [
            isValue('context.role', 'admin'),
            async ({ context }) => context?.role === 'editor',
          ],
        }),
        checkComplex: guardBatch(
          async ({ context }) => (context?.age ?? 0) >= 18,
          {
            or: [
              isValue('context.role', 'admin'),
              guardBatch({
  ...
  describe('#01 => checkAnd (AND logical gate)', () => {
    test('#01 => fails when one guard is false', async () => {
      const service = interpret(machine, {
        context: { age: 20, role: 'admin', active: false },
      });
      service.start();
      await service.send('TEST_AND');
      expect(service.state.value).toBe('idle');
    });

    test('#02 => succeeds when all guards are true', async () => {
      const service = interpret(machine, {
  ...
  describe('#02 => checkOr (AND with OR member)', () => {
    test('#01 => fails when AND member is false', async () => {
      const service = interpret(machine, {
        context: { age: 20, role: 'admin', active: false },
      });
      service.start();
      await service.send('TEST_OR');
      expect(service.state.value).toBe('idle');
    });

    test('#02 => succeeds when first OR option matches', async () => {
      const service = interpret(machine, {
        context: { age: 20, role: 'admin', active: true },
      });
      service.start();
    ...
    test('#03 => succeeds when second OR option matches', async () => {
      const service = interpret(machine, {
    ...
    test('#04 => fails when no OR option matches', async () => {
      const service = interpret(machine, {
  ...
  describe('#03 => checkComplex (nested guardBatch in OR)', () => {
    test('#01 => fails when age condition is not met', async () => {
      const service = interpret(machine, {
        context: { age: 16, role: 'admin', active: true },
      });
      service.start();
      await service.send('TEST_COMPLEX');
      expect(service.state.value).toBe('idle');
    });

    test('#02 => succeeds with first OR branch (admin)', async () => {
      const service = interpret(machine, {
    ...
    test('#03 => succeeds with second OR branch (active editor)', async () => {
      const service = interpret(machine, {
    ...
    test('#04 => fails with inactive editor', async () => {
      const service = interpret(machine, {
