...
import { _machine2, DELAY, fakeDB, machine2 } from '../../../async/src/interpreters/async/data';
...
describe('machine coverage', () => {
  ...
  const {
    ...
    useData,
    useConsole,
  } = constructTests(vi, service, ({ waiter: _waiter, sender, contexts }) => ({
  ...
  describe('TESTS', () => {
    ...
    describe('#17 => Check the service', () => {
      ...
      const array = [
        'sendPanelToUser',
        'Input, please !!',
        ...Array(11).fill(['sendPanelToUser', 'Input, please !!']).flat(),
      ];
